...
def dwave_run(path_data_key, path_in):
    ...
    match = re.search(r'run([^/]*)_B=(.*)_K=(.*)_xi=(.*)_gamma=([^/]*)', path_in)
    ...
    ])  # ensure every (i,i) occurs, ordered

    maxcouplers = len(qubo_couplers)

    if 'train' not in data_key:
        raise Exception(f'careful: datakey={data_key} => you are trying to train on a validation / test set!')

    couplerslist = [maxcouplers]
    for trycouplers in [2500, 2000, 1800, 1600, 1400, 1200, 1000, 500]:
        if maxcouplers > trycouplers:
            couplerslist.append(trycouplers)

    sampler = EmbeddingComposite(DWaveSampler())
    for couplers in couplerslist:  # try to reduce as little as necessary
    ...
    save_json(pathsub + 'info.json', response.info)

    samples = np.array([''.join(map(str, sample)) for sample in response.record['sample']])
    unique_samples, unique_idx, unique_counts = np.unique(
        samples, return_index=True, return_counts=True
    )
